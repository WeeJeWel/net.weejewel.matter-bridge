...
export default class MatterBridgeServer {
  ...
  constructor({
    ...
    this.enabledFlowIds = enabledFlowIds;
  ...
  async start() {
    ...
    // Initialize all Homey Flows
    await this.__initFlows();
  ...
  async __initFlows() {
    ...
    for (const flow of Object.values(flows)) {
      if (!this.enabledFlowIds.has(flow.id)) continue;

      await this.__initFlow(flow).catch(err => this.debug(`Error initializing flow ${flow.id} during startup: ${err.message}`));
    }
  }

  async __initFlow(flow) {
    return this.__queue.add(async () => {
      this.debug(`Initializing flow ${flow.name} (${flow.id})`);

      // Create a Matter Endpoint for the flow
      const flowEndpoint = this.flowEndpoints[flow.id] = new Endpoint(BridgedNodeEndpoint, {
        id: flow.id,
        bridgedDeviceBasicInformation: {
          nodeLabel: ellipseString(flow.name),
          vendorName: ellipseString('Homey'),
          productName: ellipseString('Flow Scene'),
          serialNumber: ellipseString(flow.id.replaceAll('-', '')),
        },
      });
      await this.aggregatorEndpoint.add(flowEndpoint);

      // Capture debug for use in the server class
      const debug = this.debug;

      // Create a Scene Server that triggers the flow
      class HomeySceneServer extends OnOffServer {
        async on() {
          await flow.trigger();
          ...
          setTimeout(async () => {
            await endpoint
              ...
              .catch((err) =>
                debug(
                  `Error resetting flow state for flow ${flow.id}: ${err.message}`,
                ),
              );
          }, 100);
        }
      ...
      const endpoint = new Endpoint(
        OnOffLightDevice.with(HomeySceneServer),
        {
          id: 'main',
          onOff: {
            onOff: false,
      ...
      await flowEndpoint.add(endpoint);
    });
  }

  async __uninitFlow(flow) {
    return this.__queue.add(async () => {
      this.debug(`Uninitializing flow ${flow.id}`);

      const flowEndpoint = this.flowEndpoints[flow.id];
      if (!flowEndpoint) return;

      // Delete the Matter Flow Endpoint
      await flowEndpoint.delete();

      delete this.flowEndpoints[flow.id];
    });
  }

  async enableFlow(flowId) {
    if (this.enabledFlowIds.has(flowId)) return;
    ...
    this.enabledFlowIds.add(flowId);
    await this.__initFlow(flow);
  }
  ...
  async disableFlow(flowId) {
    ...
    this.enabledFlowIds.delete(flowId);
    await this.__uninitFlow(flow);
  }
