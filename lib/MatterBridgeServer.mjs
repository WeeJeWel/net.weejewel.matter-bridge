  ...
  async __initDevice(device) {
    ...
    switch (deviceClass) {
      ...
      case 'light': {
        ...
        console.log(endpointClass.name);
        console.log('endpointServers', endpointServers.map(server => server.name));
        console.log('endpointProperties', endpointProperties);
      ...
      case 'curtain': {
        if (device.capabilitiesObj?.windowcoverings_set) {
          const HomeyWindowConveringServer = class extends WindowCoveringServer.with(
            ...
            async handleMovement(type, reversed, direction, targetPercent100ths) {
              await device.setCapabilityValue({
                ...
                value: scaleNumber(targetPercent100ths, 0, 10000, 0, 1),
              });
            }
          ...
          makeCapabilityInstance('windowcoverings_set', async value => {
            const percent100ths = scaleNumber(value, 0, 1, 0, 10000);
            await endpoint.set({
              windowCovering: {
                // TODO
                // currentPositionLift: percent100ths,
                // currentPositionLiftPercentage: Math.round(percent100ths / 100),
                // currentPositionLiftPercent100ths: Math.round(percent100ths),
              },
            });
          });
        }
