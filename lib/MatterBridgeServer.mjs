...
export default class MatterBridgeServer {
  ...
  async __uninitEndpoint(device) {
    return this.__queue.add(async () => {
      ...
      // Destroy all capability instances to remove event listeners
      const capabilityInstances = this.deviceCapabilityInstances[device.id];
      if (capabilityInstances) {
        for (const [capabilityId, instance] of Object.entries(capabilityInstances)) {
          try {
            instance.destroy();
          } catch (err) {
            this.debug(`Error destroying capability instance ${capabilityId} for device ${device.id}: ${err.message}`);
          }
        }
