...
export default class MatterBridgeServer {
  ...
  async start() {
    ...
    this.api.devices.on('device.delete', (device) => {
      ...
      this.__uninitDevice(device).catch((err) =>
        this.debug(`Error uninitializing device ${device.id} on delete: ${err.message}`),
      );
    });
    this.api.devices.on('device.update', (device) => {
      if (!this.enabledDeviceIds.has(device.id)) return;

      // Device has become ready, and isn't intiialized yet.
      if (device.ready === true && !this.deviceEndpoints[device.id]) {
        this.__initDevice(device).catch((err) =>
          this.debug(`Error initializing device ${device.id} on update: ${err.message}`),
        );
      }

      // Device has become unready, and was initialized.
      else if (device.ready === false && this.deviceEndpoints[device.id]) {
        this.__uninitDevice(device).catch((err) =>
          this.debug(`Error uninitializing device ${device.id} on update: ${err.message}`),
        );
      }
    });
  ...
  async __initDevices() {
    ...
    for (const device of Object.values(devices)) {
      ...
      await this.__initDevice(device).catch((err) =>
        this.debug(`Error initializing device ${device.id} during startup: ${err.message}`),
      );
    }
  }
  ...
  async __initDevice(device) {
    return this.__queue.add(async () => {
      ...
      const deviceEndpoint = new Endpoint(BridgedNodeEndpoint, {
        id: device.id,
      ...
      this.deviceEndpoints[device.id] = deviceEndpoint;
        ...
        this.deviceCapabilityInstances[device.id][capabilityId] = device.makeCapabilityInstance(
          ...
          (...props) => {
            Promise.resolve()
              ...
              .catch((err) =>
                this.debug(
                  `Error in capability instance callback for device ${device.id} capability ${capabilityId}: ${err.message}`,
                ),
              );
          },
        );
      ...
      switch (deviceClass) {
        ...
        case 'light': {
          ...
          class HomeyColorControlServer extends ColorControlServer {
            async moveToHueAndSaturation({ hue, saturation }) {
              await Promise.all([
                ...
                device.capabilitiesObj.light_mode &&
                  device.setCapabilityValue({
          ...
          if (device?.capabilitiesObj?.light_hue && device?.capabilitiesObj?.light_saturation) {
            endpointProperties.colorControl = {
              ...
              currentSaturation:
                scaleAndRoundNumber(device.capabilitiesObj?.light_saturation?.value, 0, 1, 0, 254) ?? 0,
            };
          ...
          if (device?.capabilitiesObj?.light_temperature) {
            endpointProperties.colorControl = {
              ...
              colorTemperatureMireds:
                scaleNumber(device.capabilitiesObj?.light_temperature?.value, 0, 1, 1, 1000) ?? 500,
          ...
          if (
            ...
            endpointServers.push(
              HomeyColorControlServer.with(ColorControl.Feature.HueSaturation, ColorControl.Feature.ColorTemperature),
        ...
        case 'heatpump': {
            ...
            ThermostatDevice.with(
              class extends ThermostatServer.with(...thermostatServerFeatures) {
                async setpointRaiseLower() {
                  // This method seems to be never called, yet is required to be implemented
            ...
            ),
            {
              ...
              thermostat: {
                ...
                occupiedHeatingSetpoint:
                  typeof device.capabilitiesObj?.target_temperature?.value === 'number'
                    ? Math.round(device.capabilitiesObj?.target_temperature?.value * 100)
                    : 0,
                minHeatSetpointLimit:
                  typeof device.capabilitiesObj?.target_temperature?.min === 'number'
                ...
                maxHeatSetpointLimit:
                  typeof device.capabilitiesObj?.target_temperature?.max === 'number'
          ...
          makeCapabilityInstance('target_temperature', async (value) => {
            await endpoint.set({
              thermostat: {
                occupiedHeatingSetpoint: typeof value === 'number' ? Math.round(value * 100) : null,
              },
            });
          });
          ...
          if (device.capabilitiesObj?.locked) {
            ...
            const endpoint = new Endpoint(DoorLockDevice.with(HomeyDoorLockServer), {
              ...
              doorLock: {
                ...
                lockState:
                  device.capabilitiesObj?.locked?.value === true
            ...
            makeCapabilityInstance('locked', async (value) => {
              await endpoint.set({
                doorLock: {
                  lockState: value === true ? DoorLock.LockState.Locked : DoorLock.LockState.Unlocked,
                },
              });
            });
          }
          ...
          if (device.capabilitiesObj?.windowcoverings_set) {
            ...
            makeCapabilityInstance('windowcoverings_set', async (value) => {
              await endpoint.set({
                windowCovering: {
                  currentPositionLiftPercent100ths:
                    typeof value === 'number' ? 10000 - scaleNumber(value, 0, 1, 0, 10000) : 5000,
                  targetPositionLiftPercent100ths:
                    typeof value === 'number' ? 10000 - scaleNumber(value, 0, 1, 0, 10000) : 5000,
                },
              });
            ...
            const HomeyWindowConveringServer = class extends WindowCoveringServer.with(WindowCovering.Feature.Lift) {
              async upOrOpen() {
                await device.setCapabilityValue({
        ...
        case 'sensor': {
          ...
          if (device.capabilitiesObj?.measure_co) {
            const endpoint = new Endpoint(
              SmokeCoAlarmDevice.with(CarbonMonoxideConcentrationMeasurementServer.with('NumericMeasurement')),
              {
                id: 'measure_co',
            ...
            makeCapabilityInstance('measure_co', async (value) => {
              await endpoint.set({
                carbonMonoxideConcentrationMeasurement: {
                  measuredValue: typeof value === 'number' ? Math.round(value) : null,
                },
              });
            });
          }
          ...
          if (device.capabilitiesObj?.measure_co2) {
            const endpoint = new Endpoint(
              AirQualitySensorDevice.with(CarbonDioxideConcentrationMeasurementServer.with('NumericMeasurement')),
              {
                id: 'measure_co2',
          ...
          if (device.capabilitiesObj?.measure_pm10) {
            const endpoint = new Endpoint(
              AirQualitySensorDevice.with(Pm10ConcentrationMeasurementServer.with('NumericMeasurement')),
              {
                id: 'measure_pm10',
          ...
          if (device.capabilitiesObj?.measure_pm25) {
            const endpoint = new Endpoint(
              AirQualitySensorDevice.with(Pm25ConcentrationMeasurementServer.with('NumericMeasurement')),
              {
                id: 'measure_pm25',
          ...
          if (device.capabilitiesObj?.alarm_smoke) {
            ...
            makeCapabilityInstance('alarm_smoke', async (value) => {
              await endpoint.set({
                smokeCoAlarm: {
                  smokeState: value === true ? SmokeCoAlarm.AlarmState.Critical : SmokeCoAlarm.AlarmState.Normal,
                },
              });
            });
          }
  ...
  async __uninitDevice(device) {
    return this.__queue.add(async () => {
      this.debug(`Uninitializing ${device.id}`);
