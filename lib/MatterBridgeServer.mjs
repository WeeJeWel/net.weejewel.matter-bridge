  ...
  async __initDevice(device) {
    ...
    switch (deviceClass) {
      case 'socket': {
        ...
        const endpointServers = [FixedLabelServer];
      ...
      case 'light': {
        ...
        if (device?.capabilitiesObj?.onoff
          ...
          const endpointLight = new Endpoint(ExtendedColorLightDevice.with(
            ...
            colorControl: {
              colorMode: device.capabilitiesObj?.light_mode?.value === 'temperature'
