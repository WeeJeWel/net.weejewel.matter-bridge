...
export default class MatterBridgeApp extends Homey.App {
  ...
  async onInit() {
    ...
    await this.api.flow.connect();
    await this.api.flow.getFlows();
    ...
    this.server = new MatterBridgeServer({
      ...
      enabledFlowIds: await this.getEnabledFlowIds(),
  ...
  async getEnabledFlowIds() {
    return new Set(await this.homey.settings.get('enabledFlowIds') || []);
  }
  ...
  async onAPIGetFlows() {
    const enabledFlowIds = await this.getEnabledFlowIds();
    ...
    for (const flow of Object.values(flows)) {
      const flowObj = {
        id: flow.id,
        name: flow.name,
        isSelected: enabledFlowIds.has(flow.id),
      };
  ...
  async onAPIEnableFlow({ flowId }) {
    const enabledFlowIds = await this.getEnabledFlowIds();
    if (!enabledFlowIds.has(flowId)) {
      enabledFlowIds.add(flowId);
      await this.homey.settings.set('enabledFlowIds', Array.from(enabledFlowIds));
      this.server.enableFlow(flowId).catch(err => {
        this.log(`Error enabling flow ${flowId}:`, err);
      });
    }
  }

  async onAPIDisableFlow({ flowId }) {
    const enabledFlowIds = await this.getEnabledFlowIds();
    if (enabledFlowIds.has(flowId)) {
      enabledFlowIds.delete(flowId);
      await this.homey.settings.set('enabledFlowIds', Array.from(enabledFlowIds));
      this.server.disableFlow(flowId).catch(err => {
        this.log(`Error disabling flow ${flowId}:`, err);
      });
    }
  }
